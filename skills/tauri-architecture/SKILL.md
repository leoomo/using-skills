...
description: 使用 Tauri v2 构建高性能安全桌面应用，结合 Rust 后端和现代前端。涵盖命令模式、状态管理、错误处理、安全最佳实践。
version: 1.0.0
---

# Tauri 架构开发技能

当你被要求使用 Tauri 开发桌面应用程序时，请严格遵守以下规则。

## 1. Rust 后端标准

### Command Pattern
业务逻辑必须封装在 `tauri::command` 中：
...
#[tauri::command]
pub async fn get_data(id: String, state: State<'_, AppState>) -> Result<Data, AppError> {
    let data = state.service.get_data(&id).await?;
    Ok(data)
}
```

### State Management
使用 Tauri 的 `manage` 机制：
...
涉及并发安全时使用 `Arc<Mutex<T>>` 或 `tokio::sync`：

```rust
use tokio::sync::Mutex;

#[tauri::command]
async fn async_operation(state: State<'_, Mutex<AppState>>) -> Result<String, String> {
    let mut app_state = state.lock().await;
    app_state.do_something().await
}
```

### Error Handling
严禁在 Command 中使用 `unwrap()`，定义自定义错误：
...
**AI Instruction:** 标准基线严格设为这些值 (8, 6, 4)。除非用户在聊天中明确要求调整，否则始终使用这些值作为全局变量来驱动后续特定逻辑。
...
### 2.4 创意实现 (Anti-Slop Implementation)
...
### 2.9 BENTO 范式 (The "MOTION-ENGINE" BENTO PARADIGM)
...
### 2.10 发布前检查 (FINAL PRE-FLIGHT CHECK)
...
为所有 `invoke` 调用定义 TypeScript Interface：
...
// 调用
const result = await invoke<Data>('get_data', { id: '123' });
```
...
pub async fn example_command(
    ...
    if request.id.is_empty() {
        return Err(AppError::InvalidInput("id cannot be empty".into()));
    }

    // 业务逻辑
    let value = state.service.process(&request.id).await?;
...
export async function exampleCommand(request: ExampleRequest): Promise<ExampleResponse> {
  return invoke<ExampleResponse>('example_command', { request });
}
```
...
在提供代码前，先分析：
1. **前端交互信号流** - 用户操作如何触发 IPC 调用
2. **后端逻辑处理流** - Command 如何处理并返回结果

同时提供：
- Rust 后端代码
- TypeScript 前端调用代码
- 必要的 `tauri.conf.json` 配置
