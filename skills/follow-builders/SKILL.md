...
description: AI builders digest — monitors top AI builders on X and YouTube podcasts, remixes their content into digestible summaries. Use when the user wants AI industry insights, builder updates, or invokes /follow-builders. No API keys or dependencies required — all content is fetched from a central feed.
---
...
- **Other** (Claude Code, Cursor, etc.): Non-persistent agent. Terminal closes = agent stops.
  ...
  are on-demand only (user types `/follow-builders` to get one).
...
Or you can skip this and just type /follow-builders whenever you want your digest — but it
...
Set `delivery.method` to `"stdout"`. Tell them: "No problem — just type /follow-builders
...
deliver the raw JSON. For full remixed digests, the user should use /follow-builders manually
...
there's no scheduled job. Just type /follow-builders whenever you want your digest."
...
- **On-demand only:** "Type /follow-builders anytime you want your next digest."
...
This workflow runs on cron schedule or when the user invokes `/follow-builders`.
...
When the user invokes `/follow-builders` or asks for their digest manually:
